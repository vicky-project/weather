...
(function(window, document, undefined) {
  ...
  function renderWeatherView(state) {
    ...
    '<button id="refreshWeatherBtn" class="btn btn-sm btn-outline-light" title="Refresh"><i class="bi bi-arrow-repeat"></i></button>' +
    '</div></div>' +
    '<div class="card-body">' +
    ...
    if (state.forecast && state.forecast.hourly && state.forecast.hourly.length) {
      ...
      for (var i = 0; i < state.forecast.hourly.length; i++) {
        var item = state.forecast.hourly[i];
        var pop = item.pop || 0;
        ...
        html += '<div class="forecast-hour-card" data-index="' + i + '">' +
  ...
  function renderSettingsView(state) {
    ...
    var city = (sett.city && sett.city !== null) ? String(sett.city): '';
    var lat = (sett.latitude !== undefined && sett.latitude !== null) ? String(sett.latitude): '';
    var lon = (sett.longitude !== undefined && sett.longitude !== null) ? String(sett.longitude): '';
    var notifications = sett.notifications_enabled === true;
